...
namespace PartnersDemo
{
    public partial class Form1 : Form
    {
        ...
        {
            string connection = "Server=DESKTOP-90NUGUN; Database =Partners; Trusted_Connection = True; TrustServerCertificate=true;";
            using (SqlConnection connect = new SqlConnection(connection))
            {
                try
                {
                    connect.Open();
                    string query = @"SELECT p.Id, t.PartnerType as Тип, p.Name AS Партнер, p.Director, p.Phone, p.Rating,
                SUM(pp.Quantity * prod.MinPrice) AS TotalSales
                FROM Partners p
                JOIN PartnerType t ON p.IdType = t.Id
                LEFT JOIN PartnerProduct pp ON p.Id = pp.IdPartner
                LEFT JOIN Products prod ON pp.Article = prod.Article
                GROUP BY p.Id, t.PartnerType, p.Name, p.Director, p.Phone, p.Rating";
                    using (SqlCommand command = new SqlCommand(query, connect))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                card.Controls.Add(new Label { Text = $"{reader["Тип"]} | {reader["Партнер"]}", AutoSize = true, Location = new Point(10, 10) });
                                card.Controls.Add(new Label { Text = $"{reader["Director"]}", AutoSize = true, Location = new Point(10, 40) });
                                card.Controls.Add(new Label { Text = $"+7 {reader["Phone"]}", AutoSize = true, Location = new Point(10, 60) });
                                card.Controls.Add(new Label { Text = $"Рейтинг: {reader["Rating"]}", AutoSize = true, Location = new Point(10, 80) });
                                card.Controls.Add(new Label { Text = $"{discount}%", AutoSize = true, Location = new Point(450, 10) });
