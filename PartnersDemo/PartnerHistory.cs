...
namespace PartnersDemo
{
    public partial class PartnerHistory : Form
    {
        ...
        {
            string connectionString = "Server=DESKTOP-90NUGUN; Database =Partners; Trusted_Connection = True; TrustServerCertificate=true;";
            using(SqlConnection connection = new SqlConnection(connectionString))
            {
                try
                {
                    ...
                    string query = @"Select pr.Name AS [Продукция], pp.Quantity AS [Количество], pp.Date AS [Дата реализации]
                                    FROM PartnerProduct pp JOIN Partners p ON pp.IdPartner = p.Id JOIN Products pr ON pp.Article = pr.Article
